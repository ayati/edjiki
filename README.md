...
- **全画面編集モード**（集中執筆向け。PC はホバーで ⛶、スマホはメニューから。全画面中に Drive 未保存変更があると **☁ 保存して閉じる** ボタンを表示）
...
| 全画面モード | マウス: エントリにカーソルを乗せると右上に ⛶ が現れる。スマホ・PC共通: ⋯ メニュー →「全画面モードへ」または `Ctrl+Enter`。終了は **✕ 閉じる** または `Esc`。Drive 未保存の変更がある場合は **☁ 保存して閉じる** ボタンが表示され、1タップで保存と全画面終了を同時に行える |
