...
#### `EDJIKI_DRIVE_FILE_ID`（既存ファイルを直接指定）

このアプリ以外で作成した `.txt` ファイルを使いたい場合に使用します。Drive でファイルを開き、URL の ID 部分をコピーして設定します。
...
| `drive.file` スコープの制限 | 他のアプリが作成した同名ファイルにはアクセス不可。回避策は `EDJIKI_DRIVE_FILE_ID` オプションを参照 |
