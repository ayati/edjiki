...
- **Drive 自動保存**（入力停止30秒後・アプリ切替時・全画面終了時に自動で Drive へ保存）
...
| Drive へ自動保存 | ON（デフォルト）にすると、入力が止まって30秒後・アプリ切替時・全画面終了時に自動で Drive へ保存する。Drive 未設定時は無効 |
...
| タイミング | 条件 |
|---|---|
| 入力が止まって30秒後 | Drive 認証済み・オンライン・非ロック時 |
...
| 表示 | 意味 |
|---|---|
| **☁**（タップ可能） | Drive 未認証で未保存の変更あり → タップで認証＆保存 |
| **保存中...** | Drive へ保存処理中 |
| **✓ 保存済** | 保存完了（3秒後に非表示） |
| **✕ 保存失敗** | 保存エラー → タップで再試行 |
