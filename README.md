...
デフォルトはマイドライブ直下に `edjiki.txt` として保存されます。保存先は **⋯ → Drive 設定 →「Google Drive 連携」** から変更できます。また `config.js` で初期値を指定することもできます。
...
| セクション | 内容 |
|---|---|
| Google Drive 連携 | メインデータ（`edjiki.txt`）の保存先フォルダを設定 |
...
アーカイブファイルは edjiki のテキスト形式（`YYYY/MM/DD HH:MM:SS 本文`）なので、必要に応じて **⋯ → ファイルから読込** で現行データに取り込むことができます。
...
Google Drive で任意の場所に専用フォルダを作成します（例: `jiki_archive`）。メインデータ（`edjiki.txt`）の保存フォルダとは別にすることを推奨します。
